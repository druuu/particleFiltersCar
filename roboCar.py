...
class robot:
    ...
    def check_goal(self, goal, threshold = 0.5):
        dist =  sqrt((float(goal[0]) - self.x) ** 2 + (float(goal[1]) - self.y)
** 2)
...
1000):
    ...
    # moving Robot Positions List initialization
    listRoboPos = []
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        #moving robot positions
        listRoboPos.append(myrobot.roboPos())
...
x,x1 = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
        weight_data, weight_smooth, p_gain, d_gain)
